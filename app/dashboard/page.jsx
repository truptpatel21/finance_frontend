...
export default function Home() {
  ...
  function getChartBase64(chartRef) {
  ...
  function getCompressedBase64Image(chartRef, quality = 0.5) {
    if (chartRef.current && chartRef.current.canvas) {
      return chartRef.current.canvas.toDataURL('image/jpeg', quality);
    }
    return null;
  }
  ...
  const handleDownloadReport = async () => {
    const barChartImg = getCompressedBase64Image(summaryChartRef);
    const doughnutChartImg = getCompressedBase64Image(categoryChartRef);
    ...
    try {
      ...
      if (res && res instanceof Blob) {
        ...
        a.href = url;
        a.download = `Finance_Report_${year}_${month}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        toast.success("PDF downloaded and sent to your email!");
      } else {
    ...
    `}</style>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col gap-8">
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-2">
                  <svg className="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M9 17v-6h6v6m2 4H7a2 2 0 01-2-2V5a2 2 0 012-2h6l5 5v11a2 2 0 01-2 2z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  <h2 className="text-2xl font-semibold text-purple-600">Spending Insights</h2>
                </div>
              </div>
              <div className="text-gray-700">
                <p className="mb-4">Here's a quick breakdown of how your expenses compare across top categories this month:</p>
                <ul className="list-disc pl-5 space-y-2">
                  {topCategories.slice(0, 3).map((cat, idx) => (
                    <li key={idx}>
                      <span className="font-medium">{cat.category}:</span> ₹{Number(cat.total_spent).toFixed(2)}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {userProfile?.user?.subscription_plan === "elite" && (
            <motion.div
            ...
            >
              <h2 className="text-xl font-semibold text-blue-700 mb-4">Download Custom Report</h2>
              <p className="mb-6 text-sm text-gray-600">Get a professionally formatted PDF report of your finances.</p>
              <button
                onClick={handleDownloadReport}
                className="btn-gradient px-6 py-2 rounded font-medium text-white"
              >
                Download Report
              </button>
            </motion.div>
